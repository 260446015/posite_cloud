...
//    private CachingSessionDAO sessionDao;
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) {
        ...
        User byUsername = userRepository.findByUsername(loginName);
        // 创建principal身份
//        ShiroUser su = new ShiroUser.Builder(byUsername.getId(), loginName).name(byUsername.getName()).jobLevel(byUsername.getJobLevel()).ip(host)
//                .searchCount(byUsername.getSearchCount()).totalSerachCount(byUsername.getTotalSerachCount()).builder();
        SimpleAuthenticationInfo info = new SimpleAuthenticationInfo(byUsername, token.getPassword(), getName());
